...
encho dump(p)  # dumps a person as YAML
 ```
...
(Whitespace for clarity, because we're dumping three values.)
...
Once again, we define a generic proc to handle any possible type. Not that the
variable we want to load is a `var` parameter rather than a return value. This
enables us to later call the `load` proc without explicitly give the generic
type parameter.
...
Remember how we generated two events that start end end our YAML document? Now
...
template constructScalarItem(events: YamlStream, e: untyped, body: untyped) =
  let e = events.next()
  doAssert e.kind == yamlScalar
  body

proc construct(events: YamlStream, s: var string) =
  constructScalarItem(events, e):
    s = e.scalarContent
...
For each of the simple types, we need to check whether the event we get is
actually a *scalar item*. Because we do not want to write the same code three
times, we use a template for it. The interesting thing here is that we can pass
the identifier `e` to this template which has not been defined before. This
usually cannot work in statically typed languages. However, since templates are
called at compile time, we can pass `e` as an `untyped` identifier.

Then, inside the template, we use the given `e` to define a read-only variable
with `let` and retrieve the next YAML event. Then, we ensure that this event is
indeed a *scalar event*. And finally, we just execute whatever has been given
as `body`.

As you see in the procs, the `body` is passed as code block into the template.
And, since it is untyped, it resolves the reference to `e` only after being put
in the template.
